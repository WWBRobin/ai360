  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="px-6 md:px-10 pt-10 pb-4">
        <h1 className="text-[28px] font-bold text-[#2a2724] leading-tight">发现最适合你的 AI 工具</h1>
        <p className="text-[15px] text-[#a1a1a1] mt-1.5">{totalCount} 个工具 · {platformCount} 个平台 · 独立评测</p>
      </div>
      {/* 双栏 */}
      <div className="flex">
      <AppSidebar />
      ...
      <main className="flex-1 min-w-0 px-6 md:px-10 pb-10">
        ...
        {/* 场景筛选 — mcp 下划线激活风格 */}
        <div className="flex items-center gap-6 py-1 border-b border-[#e3e0dd] overflow-x-auto scrollbar-hide">
          {SCENES.map(s => (
            <button
              key={s.slug}
            ...
            >
              {s.name}
              <span className="ml-1.5 text-[11px] text-[#a1a1a1]">{s.count}</span>
              {activeScene === s.slug && (
        ...
        <div className="flex items-center gap-6 py-1">
            ...
            return (
              <button
                ...
                className={`relative inline-flex h-10 shrink-0 items-center px-1 text-sm font-medium whitespace-nowrap transition ${
                  isActive ? 'text-[#2a2724]' : 'text-[#a1a1a1] hover:text-[#2a2724]'
                }`}
              >
                {t}
                {isActive && (
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="text-[14px] text-[#a1a1a1]">共 {filteredSkills.length} 个</span>
            <select
              value={sortBy}
              onChange={e => setSortBy(e.target.value)}
              className="text-[14px] text-[#656360] border border-[#e3e0dd] rounded-md px-2 py-1 bg-[#fcfbf9] outline-none"
            >
        ...
        {filteredSkills.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-[14px] text-[#a1a1a1]">该筛选下暂无工具</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2.5">
                    {skill.icon_url ? (
                      ...
                      <span className="w-9 h-9 rounded-[10px] bg-[#1c1a18] flex items-center justify-center text-[14px] font-bold text-white shrink-0">
                        {skill.name[0]}
                      </span>
                    )}
                    <div>
                      <div className="text-[15px] font-semibold text-[#2a2724] group-hover:text-[#1c1a18] transition">{skill.name}</div>
                      <div className="text-[11px] text-[#a1a1a1]">{skill.platform_name}</div>
                    </div>
                  </div>
                  {skill.overall_score && (
                    <span className="text-[15px] font-bold text-[#1c1a18]">{skill.overall_score.toFixed(1)}</span>
                  )}
                </div>
                <p className="text-[13px] text-[#656360] mb-2 leading-relaxed line-clamp-2">{skill.tagline}</p>
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  {skill.overall_score && <span className="tag tag-tested">AI360 实测</span>}
                  {!skill.overall_score && <span className="tag" style={{ background: '#f0ede9', color: '#a1a1a1' }}>收录未评测</span>}
                  {skill.free_quota && <span className="tag tag-free">免费</span>}
                </div>
                {skill.overall_score && (
                  <div className="flex items-center gap-4 pt-2 border-t border-[#f0ede9] text-[11px] text-[#a1a1a1]">
                    <span>上手 {skill.difficulty_score}/5</span>
                    <span>稳定 {skill.stability_score}/5</span>
                    {skill.free_quota && <span className="text-[#656360]">{skill.free_quota.substring(0, 15)}</span>}
                  </div>
                )}
              </Link>
            ))}
        ...
        {/* 评测文章 */}
        <div className="border-t border-[#e3e0dd] pt-6 pb-8">
          <div className="flex items-baseline justify-between mb-3">
            <h2 className="text-[18px] font-bold text-[#2a2724]">深度评测</h2>
            <Link href="/guide" className="text-[13px] text-[#1c1a18] hover:underline">全部 →</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {GUIDES.map(g => (
              <Link key={g.slug} href={`/guide/${g.slug}`} className="block p-3 border border-[#e3e0dd] rounded-lg hover:border-[#e3e0dd] transition group">
                <h3 className="text-[14px] font-medium text-[#2a2724] group-hover:text-[#1c1a18] mb-1 leading-snug">{g.title}</h3>
                <p className="text-[13px] text-[#a1a1a1]">{g.desc}</p>
              </Link>
            ))}
          </div>
        </div>
