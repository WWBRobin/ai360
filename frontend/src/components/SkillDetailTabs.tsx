  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <section id="features">
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4">功能亮点</h2>
        ...
        <section id="basic">
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4">基本信息</h2>
          <div className="content-card p-5">
            <dl className="divide-y divide-[var(--bg2)]">
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between py-2.5 text-sm">
      <dt className="text-[var(--fg3)]">{label}</dt>
      <dd className="text-[var(--fg)] font-medium">{value}</dd>
    </div>
  )
}
...
function InstallTab({
  ...
  return (
    <div className="space-y-8">
      ...
      {github && (
        <section>
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4">🔧 开发者信息</h2>
          <div className="content-card p-5">
            <div className="flex items-center gap-2 mb-3">
              <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden className="text-[var(--fg2)]">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs font-semibold text-[var(--fg2)] hover:text-[var(--primary)] transition truncate"
              >
                {github.full_name}
              </a>
            </div>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-[var(--bg2)] rounded-lg py-2">
                <div className="text-xs text-[var(--fg2)]">★ Star</div>
                <div className="text-sm font-bold text-[var(--fg)]">{formatStars(github.stars)}</div>
              </div>
              <div className="bg-[rgba(var(--dim-rgb),0.06)] rounded-lg py-2">
                <div className="text-xs text-[var(--fg3)]">⑂ Fork</div>
                <div className="text-sm font-bold text-[var(--fg)]">{formatStars(github.forks)}</div>
              </div>
              <div className="bg-[rgba(var(--dim-rgb),0.06)] rounded-lg py-2">
                <div className="text-xs text-[var(--fg3)]">License</div>
                <div className="text-xs font-bold text-[var(--fg)] truncate px-1">{github.license || '—'}</div>
              </div>
            </div>
          </div>
        </section>
...
function StepCard({
  ...
  return (
    <div className="content-card p-5">
      ...
      {desc && <p className="text-[13px] text-[var(--fg2)] leading-[1.7] mb-2">{desc}</p>}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="content-card p-5">
        ...
        <p className="text-sm text-[var(--fg2)] leading-[1.7] pl-8">
        ...
        <div className="content-card p-6">
          <div className="flex items-center gap-2.5 mb-3">
            ...
            {skill.guide_difficulty && (
              <span className="text-xs text-[var(--fg3)]">难度：{skill.guide_difficulty}</span>
            )}
          </div>
          <div className="prose-guide pl-8">
            <div className="whitespace-pre-wrap text-sm text-[var(--fg2)] leading-[1.7]">
              {skill.guide_content}
            </div>
          </div>
        </div>
      ...
      {skill.trial_enabled ? (
        <div className="content-card p-6">
          <h3 className="text-[15px] font-semibold text-[var(--fg)] mb-3">🧪 在线试用 {skill.name}</h3>
          <TrialBox skillId={skill.id} skillName={skill.name} />
        </div>
      ) : (
        <div className="content-card p-6 text-center">
          <span className="text-2xl">🔒</span>
          <p className="text-sm text-[var(--fg2)] mt-2">
...
function ReviewsTab({ skill }: { skill: SkillDetail }) {
  return (
    <div>
      <section>
        <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4">
          💬 评价讨论
        </h2>
    ...
    <div className="space-y-8">
      ...
      {compareRows.length > 1 && (
        <section>
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4 flex items-center gap-2">
            ⚖️ 同类对比
            <span className="text-xs font-normal text-[var(--fg3)]">向上箭头为最优值</span>
          </h2>
          <div className="content-card overflow-hidden">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-[rgba(var(--dim-rgb),0.6)]">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-[var(--fg)] uppercase tracking-wide">工具</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-[var(--fg)] uppercase tracking-wide">综合</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-[var(--fg)] uppercase tracking-wide">上手</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-[var(--fg)] uppercase tracking-wide">稳定</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-[var(--fg)] uppercase tracking-wide">免费额度</th>
                </tr>
              </thead>
              <tbody>
                {compareRows.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.slug}
                      className={`border-b border-[var(--border)] last:border-b-0 transition hover:bg-[rgba(var(--dim-rgb),0.04)] ${
                        isCurrent ? 'bg-[rgba(var(--dim-rgb),0.5)]' : ''
                      }`}
                    >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isCurrent ? (
                            <span className="font-semibold text-[var(--primary)]">
                              {row.name}
                              <span className="ml-1 text-[10px] align-middle bg-[rgba(var(--dim-rgb),0.06)] text-[var(--primary)] px-1 py-px rounded">当前</span>
                            </span>
                          ) : (
                            <Link href={`/skill/${row.slug}`} className="font-semibold text-[var(--fg)] hover:text-[var(--primary)] transition">
                              {row.name}
                            </Link>
                          )}
                          <span className="text-[11px] text-[var(--fg3)] block">{row.platform_name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-center">
                        {row.overall_score != null ? (
                          <span className="font-bold text-[var(--fg)]">{row.overall_score.toFixed(1)}</span>
                        ) : (
      ...
      {altRows.length > 0 && (
        <section>
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-1">🔁 替代品推荐</h2>
      ...
      {related.length > 0 && (
        <section>
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4">🔗 相关推荐</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {related.map((s) => (
      ...
      {relatedArticles.length > 0 && (
        <section>
          <h2 className="text-[17px] font-bold text-[var(--fg)] mb-4">📰 相关文章</h2>
      ...
      <div className="content-card p-5 h-full flex flex-col">
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="min-w-0">
            <h3 className="font-bold text-[var(--fg)] group-hover:text-[var(--primary)] transition truncate">
              {row.name}
            </h3>
            <span className="text-xs text-[var(--fg3)]">{row.platform_name}</span>
          </div>
        </div>
        {row.tagline && (
          <p className="text-sm text-[var(--fg2)] mb-3 line-clamp-2 flex-1">{row.tagline}</p>
        )}
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-[var(--primary)] font-medium group-hover:underline">查看评测 →</span>
        </div>
      </div>
    </Link>
  )
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="bg-[rgba(var(--dim-rgb),0.06)] text-[var(--primary)] text-[10px] px-1.5 py-0.5 rounded font-medium">
                {article.tag}
              </span>
            </div>
            <h3 className="font-semibold text-[var(--fg)] group-hover:text-[var(--primary)] transition line-clamp-2 text-sm">
              {article.title}
            </h3>
            <p className="text-xs text-[var(--fg2)] mt-1.5 line-clamp-2">{article.summary}</p>
            <div className="mt-2 text-xs text-[var(--primary)] font-medium">阅读 →</div>
          </div>
        </div>
      </div>
    </Link>
