    ...
    <main className="flex-1 min-w-0 px-6 md:px-10 py-8">
      {/* 标题区 */}
        <div className="mb-8 pb-6 border-b border-[#F0F0F0]">
          <nav className="text-[12px] text-[#9CA3AF] mb-3">
            <Link href="/" className="hover:text-[#FF8C00]">首页</Link>
            <span> / </span>
            <span className="text-[#6B7280]">学习中心</span>
          </nav>
          <h1 className="text-[26px] font-bold text-[#1F2937] mb-2" style={{ letterSpacing: '-0.5px' }}>
            🎓 学习中心
          </h1>
          <p className="text-[15px] text-[#4B5563] max-w-[640px] leading-relaxed">
            选择一条路径，闯关式学习。每完成一步解锁下一步，走完全程获得徽章。
          </p>

          {/* 难度说明 */}
          <div className="flex items-center gap-4 mt-4">
            <span className="text-[12px] text-[#9CA3AF]">难度分级：</span>
            {Object.entries(DIFFICULTY_META).map(([key, meta]) => (
              <span
                key={key}
                className="text-[13px] px-2.5 py-1 rounded-full"
                style={{ background: 'rgba(255,140,0,0.10)', color: '#FF8C00' }}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10">
          ...
          >
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                <h2 className="text-[18px] font-bold text-[#1F2937] group-hover:text-[#FF8C00] transition">
                  按工具学
                </h2>
                <p className="text-[13px] text-[#9CA3AF]">深入了解每个平台的玩法</p>
              </div>
            </div>
            <p className="text-[13px] text-[#4B5563] leading-relaxed">
              Hermes、扣子、Claude Skills、GPTs — 从安装到精通，每个平台一条完整路径。
            </p>
            <div className="mt-3 text-[13px] text-[#FF8C00] font-medium">
          ...
          >
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#9CA3AF]">不知道用什么工具？从需求出发</p>
              </div>
            </div>
            <p className="text-[13px] text-[#4B5563] leading-relaxed">
        ...
        <section id="tool-paths" className="mb-10 scroll-mt-20">
          <div className="flex items-center gap-2.5 mb-5">
            <h2 className="text-[18px] font-bold text-[#1F2937]">🔧 按工具学</h2>
            <span className="text-[13px] text-[#9CA3AF]">{TOOL_PATHS.length} 条路径</span>
          </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2.5">
                      ...
                      <h3 className="text-[15px] font-semibold text-[#1F2937] group-hover:text-[#FF8C00] transition">
                        {path.platformName}
                      </h3>
                    </div>
                    {done && (
                  ...
                  <p className="text-[13px] text-[#9CA3AF] mb-3 leading-relaxed">{path.description}</p>
                  {/* 进度条 */}
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-[6px] rounded-full overflow-hidden" style={{ background: '#F3F4F6' }}>
                      <div
                        className="h-full rounded-full transition-all duration-300"
                        style={{ width: `${pct}%`, background: pct === 100 ? '#10B981' : '#FF8C00' }}
                      />
                    </div>
                    <span className="text-[11px] text-[#9CA3AF] tabular-nums w-8 text-right">{pct}%</span>
                  </div>
        ...
        <section id="scene-paths" className="mb-10 scroll-mt-20">
          <div className="flex items-center gap-2.5 mb-5">
            <h2 className="text-[18px] font-bold text-[#1F2937]">🎯 按场景学</h2>
            <span className="text-[13px] text-[#9CA3AF]">{SCENE_PATHS.length} 条路径</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
