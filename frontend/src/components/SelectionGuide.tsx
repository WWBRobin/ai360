  ...
  return (
    <section className="mt-16">
      <div className="content-card p-6 md:p-8">
        <div className="mb-6">
          <h2 className="flex items-center gap-2 text-xl font-bold text-[var(--fg)]">
            💡 不知道选哪个？看这里
          </h2>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {recommendations.map((rec) => (
            <div
              key={rec.title}
              className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-4 hover:border-[var(--primary)] transition-all"
            >
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xl">{rec.icon}</span>
                <span className="text-sm font-semibold text-[var(--fg)]">{rec.title}</span>
              </div>
              {rec.skill ? (
                <Link href={`/skill/${rec.skill.slug}`} className="block group">
                  <div className="font-bold text-[var(--fg)] group-hover:text-[var(--primary)] transition text-sm mb-1 line-clamp-1">
                    {rec.skill.name}
                  </div>
                  <p className="text-xs text-[var(--fg2)] line-clamp-2 leading-relaxed">{rec.reason}</p>
                  <div className="mt-2 text-xs text-[var(--primary)] font-medium group-hover:underline">
                    查看详情 →
                  </div>
                </Link>
              ) : (
                <div>
                  <div className="text-sm text-[var(--fg3)] mb-1">暂无推荐</div>
                  <p className="text-xs text-[var(--fg2)] line-clamp-2 leading-relaxed">{rec.reason}</p>
                </div>
              )}
            </div>
        ...
        {/* 对比入口 */}
        {evaluated.length >= 2 && (
          <div className="mt-6 flex items-center justify-between rounded-xl bg-[var(--sidebar)] px-4 py-3 border border-[var(--border)]">
            <span className="text-sm text-[var(--fg2)]">📋 还在纠结？把它们放在一起对比</span>
            <Link
